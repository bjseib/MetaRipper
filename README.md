...
4. In the same panel, pick a publisher and fetch creatives from the Meta Ad Library (requires a valid token). The request uses `ad_active_status=ALL` and `ad_type=ALL` to avoid overly restrictive filters and will surface any Graph API error messages inline.
